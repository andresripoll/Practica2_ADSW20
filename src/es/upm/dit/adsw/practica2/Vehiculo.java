...
public class Vehiculo {
	...
	public void setNavegador(Navegador navegador) {
		if(navegador!=null && !navegador.getVehiculo().equals(this)) {
			throw new IllegalArgumentException("El vehiculo del navegador debe ser este vehiculo");
		}
	...
	public List<Vector> irA(Vector destino) {
		if(this.getNavegador()==null){
			throw new IllegalStateException("No tenemos navegador");
		}
		if(!this.getNavegador().getMapa().estaIncluido(destino)) {
			throw new IllegalArgumentException("El destino no esta en el mapa del navegador");
		}
		Vector aux = destino;
		this.navegador.setSecuenciaMovimientos(aux);
		List<Vector> lista= new ArrayList<Vector>();
		while(!this.navegador.finMovimiento()) {
			Tramo tramo = this.navegador.siguienteMovimiento();
			getNavegador().getMapa().mueve(this,tramo);
			this.mover(tramo.hasta(),this.getT()+tramo.tiempo());
			lista.add(tramo.hasta());
		}
		return lista;
	}
